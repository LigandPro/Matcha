...
# Usage: ./final_inference_pipeline.sh -n <exp_name> -c <exp_config> -p <paths_config> [-d <device_id>] [-s <n_samples>] [-g <gnina_script>]
...
#
# Example:
#   ./final_inference_pipeline.sh -n my_experiment -c configs/default.yaml -p configs/paths/paths.yaml -d 0 -s 40 -g /path/to/gnina.sh
...
while getopts "n:c:p:d:s:g:h" opt; do
  case $opt in
    ...
    g)
      ...
      ;;
    h)
      echo "Usage: $0 -n <exp_name> -c <exp_config> -p <paths_config> [-d <device_id>] [-s <n_samples>] [-g <gnina_script>]"
      echo ""
      echo "Options:"
      ...
      echo ""
      echo "Example:"
      echo "  $0 -n my_experiment -c configs/default.yaml -p configs/paths/paths.yaml -d 0 -s 40 -g /path/to/gnina.sh"
      exit 0
      ;;
    \?)
...
# Step 5: Compute metrics from best SDF predictions
echo "Step 5: Computing metrics from best SDF predictions..."
python scripts/compute_metrics_from_sdf.py -p $paths_config -n $exp_name --prediction-type best_minimized_predictions_${n_samples}_filtered #best_minimized_predictions_filtered
