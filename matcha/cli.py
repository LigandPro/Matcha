...
def _select_pb_filters_path(preds_root: Path, dataset_name: str, prefer_minimized: bool) -> Path:
    candidates = [
        preds_root / dataset_name / "filters_results_minimized.json",
        preds_root / dataset_name / "filters_results.json",
    ] if prefer_minimized else [
        preds_root / dataset_name / "filters_results.json",
        preds_root / dataset_name / "filters_results_minimized.json",
    ]
...
def _run_optional_scoring(
    *,
    ...
    if scorer_type != "none" and scorer_type.startswith("gnina") and not resolved_device.startswith("cuda"):
        console_obj.print(f"[bold yellow][matcha][/bold yellow] GNINA requires CUDA; skipping scoring on {resolved_device}")
        scorer_type = "none"

    if scorer_type == "none":
        return scorer_used, scorer_type, scoring_sec, scorer_name

    scoring_start = time.perf_counter()
    try:
        scorer = create_scorer_fn(
            scorer_type,
        ...
        filters_path = _select_pb_filters_path(
            preds_root,
            dataset_name,
            prefer_minimized=scorer_type.startswith("gnina") and scorer_minimize,
        )

        if scorer_type.startswith("gnina") and batch_mode and gnina_batch_mode not in {"combined", "per-ligand"}:
        ...
        if scorer_type.startswith("gnina") and batch_mode and gnina_batch_mode == "combined":
            ...
            if scorer_minimize:
                compute_fast_filters_fn(conf, run_name, sdf_type="minimized", n_preds_to_use=n_samples)
        ...
        scorer_used = True
        console_obj.print(f"[bold green][matcha][/bold green] scoring complete ({scorer.name})")
    except RuntimeError as e:
        console_obj.print(f"[bold yellow][matcha][/bold yellow] scoring skipped: {e}")
...
def run_matcha(
    ...
    num_steps: int = typer.Option(10, "--num-steps", help="Number of denoising steps to run per stage."),
    export_trace: bool = typer.Option(False, "--export-trace/--no-export-trace", help="Export per-step ligand trajectories for visualization."),
) -> None:
    if out is None:
...
"""
    ...
    pipeline_timings = run_v2_inference_pipeline(
        ...
        num_steps=num_steps,
        capture_trajectory=export_trace,
    )
    ...
    scorer_used, scorer_type, scoring_sec, scorer_name = _run_optional_scoring(
        scorer_type=scorer_type,
        resolved_device=resolved_device,
        create_scorer_fn=create_scorer,
        scorer_path=scorer_path,
        scorer_minimize=scorer_minimize,
        ...
        best_scored_dir=best_scored_dir,
        ...
        conf=conf,
        run_name=run_name,
        compute_fast_filters_fn=compute_fast_filters_from_sdf,
        console_obj=console,
    )
    ...
    filters_json_path = _select_pb_filters_path(
        preds_root,
        dataset_name,
        prefer_minimized=scorer_used and scorer_type.startswith("gnina") and scorer_minimize,
    )
    pb_filters = {}
