...
def _split_multi_sdf(sdf_path: Path) -> List[Tuple[str, Any]]:
    ...
    for idx, mol in enumerate(suppl):
        if mol is None:
            console.print(
                f"[yellow]Warning: Failed to read molecule {idx} from {sdf_path.name}[/yellow]")
            continue
...
def _split_ligand_file(ligand_file: Path) -> List[Tuple[str, Any]]:
    ...
    if suffix == ".mol2":
        ...
        if mol is None:
            console.print(
                f"[yellow]Warning: Failed to read ligand file {ligand_file.name}[/yellow]")
            return []
    ...
    for uid in molecule_uids:
        for conf_idx in range(n_samples):
            pocket_centers[f"{uid}_mol0_conf{conf_idx}"] = [
                {"tr_pred_init": ligand_center, "full_protein_center": protein_center}]
...
def _ensure_checkpoints(path: Path) -> Path:
    ...
    if stage1.exists():
        return path
    console.print(
        f"[bold yellow][matcha][/bold yellow] checkpoints not found, downloading to {path} ...")
    path.mkdir(parents=True, exist_ok=True)
    snapshot_download(repo_id="LigandPro/Matcha",
                      local_dir=str(path), local_dir_use_symlinks=False)
    if not stage1.exists():
        raise typer.BadParameter(f"Failed to download checkpoints into {path}")
    console.print(
        f"[bold green][matcha][/bold green] checkpoints ready at {path}")
    return path
...
def _autobox_from_ligand(ligand: Path) -> Tuple[float, float, float]:
    ...
    if mol is None or mol.GetNumConformers() == 0:
        raise typer.BadParameter(
            f"Failed to read ligand for autobox: {ligand}")
...
def run_matcha(
    receptor: Optional[Path] = typer.Option(
        None, "-r", "--receptor", help="Protein structure (.pdb)."),
    ligand: Optional[Path] = typer.Option(
        None, "-l", "--ligand", help="Ligand with 3D coords (.sdf/.mol/.mol2/.pdb)."),
    ligand_dir: Optional[Path] = typer.Option(
        None, "--ligand-dir", help="File/dir with multiple ligands (.sdf/.mol2)."),
    out: Optional[Path] = typer.Option(
        None, "-o", "--out", help="Output directory."),
    checkpoints: Optional[Path] = typer.Option(
        None, "--checkpoints", help="Folder containing Matcha checkpoints (optional)."),
    config: Optional[Path] = typer.Option(
        None, "--config", help="Optional base config to merge with defaults."),
    workdir: Optional[Path] = typer.Option(
        None, "--workdir", help="Working dir (defaults to --out)."),
    center_x: Optional[float] = typer.Option(
        None, "--center-x", "--center_x", help="X coordinate of box center (Å)"),
    center_y: Optional[float] = typer.Option(
        None, "--center-y", "--center_y", help="Y coordinate of box center (Å)"),
    center_z: Optional[float] = typer.Option(
        None, "--center-z", "--center_z", help="Z coordinate of box center (Å)"),
    autobox_ligand: Optional[Path] = typer.Option(
        None, "--autobox-ligand", "--autobox_ligand", help="Reference ligand file for autobox (.sdf/.mol/.pdb)"),
    run_name: str = typer.Option(
        "matcha_cli_run", "--run-name", help="Name for this docking run."),
    n_samples: int = typer.Option(
        20, "--n-samples", help="Number of samples (poses) to generate per ligand."),
    n_confs: Optional[int] = typer.Option(
        None, "--n-confs", help="Number of ligand conformers to generate with RDKit (default min(10, n-samples))."),
    device: Optional[str] = typer.Option(
        None, "--device", "-g", "--gpu", help="Device: auto, cpu, cuda, cuda:N, or mps."),
    overwrite: bool = typer.Option(
        False, "--overwrite", help="Remove existing run folder if present."),
    keep_workdir: bool = typer.Option(
        False, "--keep-workdir/--no-keep-workdir", help="Keep working data (default: False)."),
    log: Optional[Path] = typer.Option(
        None, "--log", help="Path to log file (defaults to <out>/<run-name>.log)."),
    docking_batch_limit: int = typer.Option(
        15000, "--docking-batch-limit", help="Number of tokens per batch for docking models."),
    scorer_type: str = typer.Option(
        "gnina", "--scorer", help="Pose scorer: gnina, custom, or none."),
    scorer_path: Optional[Path] = typer.Option(
        None, "--scorer-path", help="Path to gnina binary or custom scorer script."),
    scorer_minimize: bool = typer.Option(
        True, "--scorer-minimize/--no-scorer-minimize", help="Minimize poses during scoring (gnina)."),
    physical_only: bool = typer.Option(False, "--physical-only/--keep-all-poses",
                                       help="Keep only PoseBusters-passing poses in outputs (default: False)."),
    num_dataloader_workers: int = typer.Option(
        32, "--num-dataloader-workers", help="Number of workers for dataloader."),
) -> None:
    if out is None:
    ...
    if device is None or device == "auto":
        ...
        console.print(
            f"[bold red]Error:[/bold red] Unknown device '{device}'. Use auto, cpu, cuda, cuda:N, or mps.")
        raise typer.Exit(code=1)

    if resolved_device == "mps" and not (hasattr(torch.backends, "mps") and torch.backends.mps.is_available()):
        console.print(
            "[bold red]Error:[/bold red] MPS (Apple Metal) requested but not available.")
        raise typer.Exit(code=1)
    if resolved_device == "cuda" and not torch.cuda.is_available():
        console.print(
            "[bold red]Error:[/bold red] CUDA requested but not available.")
        raise typer.Exit(code=1)
...
"""
    ...
    if manual_box_specified and autobox_specified:
        raise typer.BadParameter(
            "Cannot use both manual box and autobox. Choose one method.")
    if manual_box_specified:
        if center_x is None or center_y is None or center_z is None:
            raise typer.BadParameter(
                "Manual box requires --center-x/--center-y/--center-z")
        box_center_val = (center_x, center_y, center_z)
        console.print(
            f"[bold green][matcha][/bold green] manual center: {box_center_val}")
    elif autobox_specified:
        if not autobox_ligand.exists():
            raise typer.BadParameter(
                f"Autobox ligand file not found: {autobox_ligand}")
        box_center_val = _autobox_from_ligand(autobox_ligand)
        console.print(
            f"[bold green][matcha][/bold green] autobox from reference ligand {autobox_ligand.name}")
        console.print(
            f"[bold green][matcha][/bold green] center: {box_center_val}")
    else:
        console.print(
            f"[bold yellow][matcha][/bold yellow] No box specified - running blind docking on entire protein")
    ...
    if batch_mode:
        console.print(
            f"[bold cyan][matcha][/bold cyan] Batch mode: processing multiple molecules from {ligand_dir.name}")
        if ligand_dir.is_file():
            if ligand_dir.suffix.lower() not in {".sdf", ".mol2"}:
                raise typer.BadParameter(
                    "--ligand-dir file input supports .sdf or .mol2")
            molecules = _split_ligand_file(ligand_dir)
        elif ligand_dir.is_dir():
            molecules = []
            ligand_files = sorted(
                list(ligand_dir.glob("*.sdf")) +
                list(ligand_dir.glob("*.mol2")),
            ...
            raise typer.BadParameter(
                "--ligand-dir must be a file or directory")
        if not molecules:
            raise typer.BadParameter(f"No molecules found in {ligand_dir}")
        console.print(
            f"[bold green][matcha][/bold green] Found {len(molecules)} molecules to process")
        molecule_uids = _prepare_batch_dataset(
            receptor, molecules, dataset_dir)
    else:
        _prepare_singleton_dataset(
            receptor_for_run, ligand, dataset_dir, run_name)
    ...
    if box_center_val is not None:
        ...
        _create_pocket_centers_file(
            box_center_val, molecule_uids, n_samples, pocket_centers_filename)
    ...
    console.print(
        f"[bold green][matcha][/bold green] checkpoints: {checkpoints}")
    console.print(
        f"[bold green][matcha][/bold green] samples per ligand: {n_samples}")
    console.print(
        f"[bold green][matcha][/bold green] device: {resolved_device}")
    ...
    if scorer_type != "none":
        try:
            ...
            scorer.score_poses(str(receptor), str(sdf_input),
                               str(sdf_scored), device=cuda_device_idx)

            compute_fast_filters_from_sdf(
                conf, run_name, sdf_type='minimized', n_preds_to_use=n_samples)
            ...
            scorer_used = True
            console.print(
                f"[bold green][matcha][/bold green] scoring complete ({scorer.name})")
        except RuntimeError as e:
            console.print(
                f"[bold yellow][matcha][/bold yellow] scoring skipped: {e}")

    metrics = np.load(
        preds_root / f"{dataset_name}_final_preds_merged.npy", allow_pickle=True).item()
    ...
    if scorer_used and sdf_scored.exists():
        for uid_key, uid_data in metrics.items():
            uid_real = uid_key.split(
                '_mol')[0] if '_mol' in uid_key else uid_key
            scored_sdf = sdf_scored / f"{uid_real}.sdf"
            if not scored_sdf.exists():
                continue
            suppl = Chem.SDMolSupplier(
                str(scored_sdf), removeHs=False, sanitize=False)
            for i, mol in enumerate(suppl):
                ...
                for prop in ['minimizedAffinity', 'Affinity', 'minimizedCNNscore', 'CNNscore']:
                    if mol.HasProp(prop):
                        try:
                            uid_data['sample_metrics'][i]['gnina_score'] = float(
                                mol.GetProp(prop))
                            break
                        except (ValueError, TypeError):
                            continue
    ...
    def _rank_samples(sample_metrics: List[dict]) -> List[Tuple[int, dict]]:
        ...
        ranked_indices = sorted(
            ...
            key=lambda i: (
                ...
                float(sample_metrics[i].get("gnina_score",
                      float("inf"))) if has_gnina else 0,
            ),
        )
    ...
    def _save_all_poses_for_uid(metrics_data, uid, out_path, filter_non_physical: bool = True):
        ...
        best_pb_count = max([int(s.get("posebusters_filters_passed_count_fast", 0))
                            for s in sample_data["sample_metrics"]])

        # Optionally filter out poses failing PoseBusters fast checks
        ranked_filtered = [(r, s) for r, s in ranked if not filter_non_physical or int(
            s.get("posebusters_filters_passed_count_fast", 0)) == best_pb_count]
    ...
    def _get_best_sample_idx(pb_counts, gnina_scores=None):
        ...
        pb_count_indices = np.arange(len(pb_counts))[
            pb_counts == best_pb_count]
    ...
    def _get_sample_stats(sample_metrics):
        pb_counts = np.array(
            [int(s.get("posebusters_filters_passed_count_fast", 0)) for s in sample_metrics])
        has_gnina = any('gnina_score' in s for s in sample_metrics)
        gnina_scores = None
        if has_gnina:
            gnina_scores = np.array(
                [float(s.get("gnina_score", float("inf"))) for s in sample_metrics])
    ...
    def _format_pose_ranking_lines(ranked_samples, has_gnina):
        ...
        for rank, sample in ranked_samples:
            ...
            if has_gnina:
                ...
                lines.append(
                    f"  {rank:<4}  {aff}  {pb_count}/4  {checks}   {buried_frac:>6}")
            else:
                lines.append(
                    f"  {rank:<4}  {pb_count}/4  {checks}   {buried_frac:>6}")
        lines.extend([
            "",
    ...
    def _format_summary_section(n_samples, scorer_type, scorer_used, scorer_name,
        ...
        lines = [
            ...
            f"  Scorer                 : {scorer_type}" +
            (f" ({scorer_name})" if scorer_used else ""),
        ]
        if has_gnina:
            lines.append(
                f"  GNINA Affinity (kcal/mol): min={min(gnina_scores):.2f}, mean={float(np.mean(gnina_scores)):.2f}, max={max(gnina_scores):.2f}")
    ...
    if not batch_mode:
        ...
        pb_counts, gnina_scores, has_gnina, best_idx = _get_sample_stats(
            mdata["sample_metrics"])

        scored_path = best_scored_dir / \
            f"{run_name}.sdf" if scorer_used else None
        _write_best_pose_sdf(mdata, best_idx, uid,
                             resolved_out, scored_path=scored_path)

        ranked_samples, kept_physical, total_samples = _save_all_poses_for_uid(
            metrics, uid, all_poses_dest, filter_non_physical=physical_only)
        ...
        command_line = "uv run matcha " + \
            " ".join(
                sys.argv[1:]) if ".venv/bin/matcha" in sys.argv[0] else " ".join(sys.argv)
        log_lines = [
            banner.rstrip("\n"),
        ...
        log_lines.extend(_format_box_log_section(
            box_center_val, autobox_specified, autobox_ligand))
        log_lines.extend([
            "[ INPUT / OUTPUT FILES ]",
        ...
        if max(pb_counts) < 4:
            console.print(
                "[bold yellow]WARNING: No poses passed all PoseBusters checks (pb_4/4).[/bold yellow]")
            console.print("")
        ...
        if not keep_workdir:
            ...
            console.print(
                f"[bold green][matcha][/bold green] cleaned workdir {work_dir}")
        else:
            console.print(
                f"[bold green][matcha][/bold green] keeping workdir at {work_dir}")
        return

    # batch mode output
    molecule_uids = sorted(
        [p.name for p in (work_dir / "datasets" / "any_conf").iterdir() if p.is_dir()])
    receptor_abs = receptor.resolve()
    ...
    for mol_uid in molecule_uids:
        ...
        if metrics_key not in metrics:
            console.print(
                f"[yellow]Warning: No results for {mol_uid}[/yellow]")
            continue
        ...
        scored_path = best_scored_dir / \
            f"{mol_uid}.sdf" if scorer_used else None
        _write_best_pose_sdf(mdata, best_idx, mol_uid,
                             best_dest, scored_path=scored_path)

        ranked_samples, kept_physical, total_samples = _save_all_poses_for_uid(
            metrics, metrics_key, all_dest, filter_non_physical=physical_only
        )

        ligand_input = (run_workdir / "work" / "datasets" /
                        "any_conf" / mol_uid / f"{mol_uid}_ligand.sdf").resolve()
